...
代码的主要流程：
定义了一个main函数，接受命令行参数作为输入。
在main函数中，首先调用setup_mpi函数设置MPI环境。
创建一个ClutterRemovalSim对象，用于模拟场景和抓取操作。
根据命令行参数设置抓取点的数量和进度条。
如果是MPI的主进程（rank为0），创建一个目录用于保存生成的数据，并调用write_setup函数将模拟场景的设置信息写入文件。

循环生成抓取点的场景，每个场景包括以下步骤：
随机生成物体数量，并重置模拟场景。
渲染合成深度图像，并获取相机外参。
使用一部分图像重建点云。
对点云进行裁剪，去除表面和边界。
