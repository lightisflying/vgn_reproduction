"""
用于处理几何数据的ROS节点，用于将几何数据转换为ROS消息格式，并进行ROS中的坐标变换。

to_point_msg(position): 将一个numpy数组转换为geometry_msgs.msg.Point消息类型。

from_point_msg(msg): 将一个geometry_msgs.msg.Point消息类型转换为numpy数组。

to_vector3_msg(vector3): 将一个numpy数组转换为geometry_msgs.msg.Vector3消息类型。

from_vector3_msg(msg): 将一个geometry_msgs.msg.Vector3消息类型转换为numpy数组。

to_quat_msg(orientation): 将一个Rotation对象转换为geometry_msgs.msg.Quaternion消息类型。

from_quat_msg(msg): 将一个geometry_msgs.msg.Quaternion消息类型转换为Rotation对象。

to_pose_msg(transform): 将一个Transform对象转换为geometry_msgs.msg.Pose消息类型。

to_transform_msg(transform): 将一个Transform对象转换为geometry_msgs.msg.Transform消息类型。

from_transform_msg(msg): 将一个geometry_msgs.msg.Transform消息类型转换为Transform对象。

to_color_msg(color): 将一个numpy数组转换为std_msgs.msg.ColorRGBA消息类型。

to_cloud_msg(points, intensities=None, frame=None, stamp=None): 将一组无结构点的坐标转换为sensor_msgs.msg.PointCloud2消息类型。

TransformTree类: 用于进行坐标变换的类，通过ROS的tf2库实现。它包含了查找坐标变换和广播坐标变换的功能。

PS:在Colab调试模拟抓取的代码时其实没太用到，可扩展[TODO]
"""
